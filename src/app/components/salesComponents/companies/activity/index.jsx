...
import { ReactComponent as EmailIcon } from 'src/assets/svg/activityEmail.svg';
import { ReactComponent as MeetingIcon } from 'src/assets/svg/activityMeetings.svg';
...
// Removed companyActivityAction import since icon mapping handles types directly
import { useStyles } from './activity';
import LogMeetings from './logMeetings';
...
const Activity = ({ activity }) => {
  ...
  const isMeetingOrTask =
    notifiableType === notifiableTypes.meeting || notifiableType === notifiableTypes.task;

  const showExpand = description?.length > TRUNCATE_LENGTH || isMeetingOrTask;
    ...
    if (isMeetingOrTask) {
      ...
      return (
        <Box className={classes.expandableSection}>
          ...
          {notifiableType === notifiableTypes.meeting && (
            <LogMeetings
              meetingLink={activity?.meetingLink}
              dateLabel={activity?.meetingDate}
              timeLabel={activity?.meetingTime}
              description={description}
              guests={activity?.meetingGuests}
            />
          )}
        </Box>
      );
